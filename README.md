...
Les flèches ont posé le même problème pour une autre raison : pour Alt+flèche le
terminal doit encoder le modificateur dans la séquence CSI et le mien ne le fait
pas, tmux ne reçoit donc rien de modifié et c'est readline qui hérite de
l'échappement (le curseur saute d'un mot au lieu de changer de pane). La
navigation est passée sur hjkl, qui ne sont qu'un seul octet ASCII derrière
l'ESC, sans ambiguïté possible.
...
Un simple `cp tmux/.tmux.conf ~/.tmux.conf` fait l'affaire. Puis `tmux`
pour lancer une session, et `tmux a` pour rattacher la précédente.

## Raccourcis

| Hyprland | tmux | Action |
| --- | --- | --- |
| `Super+flèches` | `Alt+h/j/k/l` | changer de pane |
| `Super+Shift+flèches` | `Alt+H/J/K/L` | déplacer le pane, le focus suit |
| `Super+Ctrl+flèches` | préfixe puis `h/j/k/l` | redimensionner, répétable |
...
Le redimensionnement est sur le préfixe et pas sur Ctrl+Alt+hjkl, parce que
Ctrl+h vaut 0x08 soit exactement Backspace, et Ctrl+j vaut 0x0A soit Entrée :
tmux ne peut pas les distinguer, et on retomberait sur le même problème que les
flèches.
...
combinaison dans un pane affiche la séquence brute reçue : `^[é` signifie que
c'est transmis et que le bind fonctionnera.
